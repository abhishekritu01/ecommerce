...
export async function POST(request: Request) {
    ...
    if (data.is_final === true && data.status === 'paid') {
        ...
        // delivery person
        await db
            .update(diliveryPersons)
            .set({ orderID: sql`NULL` })
            .where(eq(diliveryPersons.orderID, parseInt(clonedData.order_id)));

        // Inventory
        await db
            .update(inveventories)
            .set({ orderId: sql`NULL` })
            .where(eq(inveventories.orderId, parseInt(clonedData.order_id)));
